...
assert.equal(model.htmlColor("#644AC9"), "#644AC9");
assert.equal(model.htmlColor("#ff644ac9"), "#644ac9");
assert.equal(model.htmlColor({ r: 100 / 255, g: 74 / 255, b: 201 / 255 }), "#644ac9");
assert.equal(model.htmlColor("not-a-color"), "");
const coloredNotes = model.eventNotesHtml(meetNotesEvent, "#644AC9");
assert.match(coloredNotes, /<font color="#644AC9">https:\/\/meet\.google\.com\/ssq-outo-obp<\/font>/);
assert.match(coloredNotes, /<font color="#644AC9">https:\/\/tel\.meet\/ssq-outo-obp\?pin=1<\/font>/);
...
assert.equal(model.showOpenInChroncalEnabled(null), false);
assert.equal(model.showOpenInChroncalEnabled({}), false);
assert.equal(model.showOpenInChroncalEnabled({ showOpenInChroncal: "Off" }), false);
assert.equal(model.showOpenInChroncalEnabled({ showOpenInChroncal: false }), false);
assert.equal(model.showOpenInChroncalEnabled({ showOpenInChroncal: "On" }), true);
